...
class TestMessagesEndpoint(unittest.TestCase):
    ...
    @async_test
    async def test_01__send__sending_a_message__response_object_returned_with_success(self):
        response = await self.client.messaging.send(username="connector_123", text="hi there!")
        print(response)
